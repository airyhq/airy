...
class InputComponent extends Component<InputProps & WithTranslation, IState> {
  ...
  checkWithHTML5Validation = inputElement => {
    ...
    if (inputElement.type === 'email') {
      if (!inputElement.validity.valid) {
        this.setState({validationResult: this.props.t('input.wrongEmail') as string});
      } else {
        this.setState({validationResult: true});
      }
    } else {
      this.setState({validationResult: this.translateResult(inputElement.type, inputElement.validity)});
    }
  };
  ...
  checkWithURLValidation = inputElement => {
    ...
    if (!inputElement.value.match(/.*\w\.\w.*/)) {
      this.setState({validationResult: this.props.t('input.urlWrongFormat') as string});
