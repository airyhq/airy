        ...
        <DialogCustomizable
            ...
            <div className={styles.content}>
                ...
                <div className={styles.filterColumn}>
                    ...
                    <div className={styles.sourcesList}>
                        {sortBy(channels, channel => channel.metadata?.name)
                            .filter((channel: Channel) => channel.metadata?.name.toLowerCase().includes(channelSearch.toLowerCase()))
                            .map((channel, key) => (
                                <div
                                    key={key}
                                    ...
                                    <div
                                        className={styles.itemName}>{channel.metadata?.name || channel.sourceChannelId}</div>
                                </div>
                            ))}
                    </div>
                </div>
                ...
                <div className={styles.filterColumn}>
                    <h3>By Source</h3>
                    <div className={styles.sourcesList}>
                        {Array.from<string>(sources).sort()
                            .map((source) => (
                                <div
                                    key={source}
                                    className={`${styles.sourceEntry} ${
                                    ...
                                    {isSourceSelected(filter.bySources, source) ? (
                                        ...
                                        <SourceIcon style={{height: '24px', width: '24px', marginRight: '4px'}}
                                                    source={source}/>
                                    )}
